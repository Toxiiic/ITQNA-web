<template>
    <div class="answer">
        <div class="author">
            <img class="avatar" :src="answerData.user_head_url" alt="">
            <span class="name">{{answerData.user_name}}</span>
            <span class="motto">{{answerData.user_motto}}</span>
        </div>
        <router-link class="title" v-if="showQuestion" :to="'/question/'+answerData.qs_id">
...
export default {
  ...
  data() {
    return {
      like_count: this.answerData.like_count
    };
  },
  methods: {
    onClickLike() {
      userLikeAnswer.create(
        {
          answer_id: this.answerData.ans_id,
          user_id: this.userId
        },
        res => {
          this.like_count++;
